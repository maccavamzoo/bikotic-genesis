...
function formatTime(ms: number, decimals: 2 | 3 = 2): string {
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-4">
          {lastLapTime !== null ? (
            <>
              ...
              <div
                className="text-white font-mono font-bold leading-none text-center"
        ...
        {laps.length >= 2 && (
          <div className="text-center pb-2">
            <span className="text-gray-600 font-mono text-sm">
              Lap {laps.length - 1}: {formatTime(laps[laps.length - 2].splitMs)}
            </span>
          </div>
        )}
    ...
    return (
      <div className="max-w-2xl mx-auto p-4 md:p-8">
        ...
        <p className="text-gray-500 mb-6">
          {laps.length} laps · Total time: {formatTime(elapsed)}
        </p>
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          <div className="bg-green-50 border border-green-200 rounded-lg p-3 text-center">
            ...
            <div className="text-lg font-mono font-bold text-green-800">{formatTime(fastest)}</div>
          </div>
          <div className="bg-gray-50 border border-gray-200 rounded-lg p-3 text-center">
            <div className="text-xs text-gray-500 font-semibold uppercase tracking-wider mb-1">Average</div>
            <div className="text-lg font-mono font-bold text-gray-800">{formatTime(average)}</div>
          </div>
          <div className="bg-red-50 border border-red-200 rounded-lg p-3 text-center">
            <div className="text-xs text-red-600 font-semibold uppercase tracking-wider mb-1">Slowest</div>
            <div className="text-lg font-mono font-bold text-red-800">{formatTime(slowest)}</div>
          </div>
        </div>
      ...
      <div className="bg-white border-2 border-gray-200 rounded-lg p-6 md:p-8 mb-6">
        <div className="space-y-4 text-gray-600 mb-8">
          ...
          <div className="flex items-start gap-3">
            ...
            <span>The last lap time displays <strong>big enough to read at a glance</strong> — already rounded to hundredths.</span>
          </div>
          <div className="flex items-start gap-3">
